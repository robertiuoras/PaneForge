...
export function placeNewPane(i: PlaceInput): Placement {
  ...
  // Only the top of the scale moves work at start. 'warn' is what a 16 GB desk with a
  // handful of panes reads most of the day, and on 2026-09-03 it sent every briefed pane
  // to the PC: "its starting session remotely immediately when i open new session on mac".
  // At 'warn' the sleep rung (shared/reclaim.ts, TIGHT_SLEEP_MINUTES) is already giving
  // finished panes' memory back, which is what makes room here; only a desk that is
  // measured OUT of memory has nothing left to give.
