...
export const DEFAULT_DONE =
  "say what changed and name the command, flow or file that proves it";

/** The `Done means:` heading. Every forged prompt ends with this block. */
const DONE_HEAD = "Done means:";
...
export function forgePrompt(input: ForgeInput): string {
  ...
  const doneBlock = [
    DONE_HEAD,
    ...(done.length ? done : [DEFAULT_DONE]).map((s) => `- ${s}`),
  ].join("\n");
  ...
  return build(
    0,
    false,
    task.slice(0, Math.max(0, task.length - over - 2)).trimEnd() + "…",
  );
}
 ...
 */
export function readPromptlibTemplate(
  id: string,
  markdown: string,
): ForgeTemplate | null {
  const examples = [
    ...fencedUnder(markdown, /^Template/i),
    ...fencedUnder(markdown, /^Harvested/i).reverse(),
  ];
