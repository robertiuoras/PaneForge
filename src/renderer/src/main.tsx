...
async function start(): Promise<void> {
   ...
   * native view then failed to attach, main removes the class itself, which puts
   * every surface back to the colour it has always been.
   */
  const g = (window as { __pfGlass?: { on: boolean } }).__pfGlass
  if (desk && g?.on) document.documentElement.classList.add('glass')
