...
driver: it opens `/Applications/PaneForge.app` and quits, whichever is newer
...
Needs a window: `test:autoclearlag`, `test:view`, `test:stashdrag`, `test:activate`, `test:restorefix`,
`test:askclick`, `test:askrender`, `test:devicesfit`, `test:phoneview`, `test:contrast`,
`test:renderwatchlive`, `test:panefit`, `test:railtrack`.
