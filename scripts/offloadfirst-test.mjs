...
// Widened 2026-09-11 after s33-mtwdutha ("close onedrive remove from mac not needed clean
// up") was moved to the Windows PC by budgetPlan, where OneDrive-on-the-Mac cannot be
// touched - the old list only caught "on my/this mac", not "remove FROM mac" or a bare
// machine-housekeeping word.
ok(pinned('close onedrive remove from mac not needed clean up'), 'the s33 incident text pins')
ok(pinned('remove onedrive off my mac'), 'off my mac pins')
ok(pinned('uninstall onedrive from the mac'), 'from the mac pins')
ok(pinned('do the cleanup on the mac'), 'on the mac pins')
ok(pinned("clean up my mac's login items"), "my mac's pins")
ok(pinned('uninstall this old app'), 'uninstall pins')
ok(pinned('add this to launchd'), 'launchd pins')
ok(pinned('check launchctl for the job'), 'launchctl pins')
ok(pinned('remove it from login items'), 'login items pins')
ok(pinned('turn it off in system settings'), 'system settings pins')
ok(pinned('reveal it in finder'), 'finder pins')
ok(pinned('unpin it from the dock'), 'the dock pins')
ok(pinned('remove the icon from the menu bar'), 'menu bar pins')
ok(pinned('update it with homebrew'), 'homebrew pins')
ok(pinned('brew uninstall the old version'), 'brew pins')
ok(pinned('clear the saved password from keychain'), 'keychain pins')
is(pinned('clean up the onboarding flow'), undefined, 'cleanup with no machine word travels')
is(pinned('remove the mac address column'), undefined, 'a mac address is not this machine')
