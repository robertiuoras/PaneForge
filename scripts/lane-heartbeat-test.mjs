...
try {
  assert.equal((await heartbeat(['first'])), null, 'directory establishment is conservative on its first heartbeat')
  assert.deepEqual((await heartbeat(['first'])), new Set(['first']), 'the next heartbeat reads the established per-process file')

  const legacy = join(git, 'paneforge-panes.json')
  writeFileSync(legacy, JSON.stringify({ 'legacy-peer': { at: Date.now(), chats: ['legacy-chat'] } }))
  assert((await heartbeat(['ours'])).has('legacy-chat'), 'a live legacy inventory remains readable during rollout')
  ...
  assert.equal((await heartbeat(['ours'])), null, 'malformed legacy inventory fails closed')
  writeFileSync(legacy, JSON.stringify({}), 'utf8')

  writeFileSync(join(dir, 'pf-not-a-pid.json'), JSON.stringify({ at: Date.now(), chats: [] }))
  assert.equal((await heartbeat(['ours'])), null, 'a malformed per-process filename fails closed')
  rmSync(join(dir, 'pf-not-a-pid.json'))
  writeFileSync(join(dir, 'pf-444.json'), '{bad', 'utf8')
  assert.equal((await heartbeat(['ours'])), null, 'a malformed per-process beat fails closed')
  rmSync(join(dir, 'pf-444.json'))

  writeFileSync(join(dir, 'pf-interrupted.tmp'), '{partial', 'utf8')
  assert((await heartbeat(['ours'])).has('ours'), 'an interrupted temp file does not strand a future writer')

  const legacyLive = await child()
  writeFileSync(legacy, JSON.stringify({ [`pf-${legacyLive.pid}`]: { at: old, chats: ['legacy-sleeping'] } }))
  assert((await heartbeat(['ours'])).has('legacy-sleeping'), 'a stale legacy beat keeps a process-confirmed live peer')
  legacyLive.kill()
  await new Promise((done) => legacyLive.once('close', done))
  assert(!(await heartbeat(['ours'])).has('legacy-sleeping'), 'a stale legacy beat expires after its process exits')
  writeFileSync(legacy, JSON.stringify({}), 'utf8')

  const live = await child()
  writeFileSync(join(dir, `pf-${live.pid}.json`), JSON.stringify({ at: old, chats: ['sleeping-chat'] }))
  assert((await heartbeat(['ours'])).has('sleeping-chat'), 'a stale timestamp keeps a process-confirmed live peer')
  live.kill()
  await new Promise((done) => live.once('close', done))
  assert(!(await heartbeat(['ours'])).has('sleeping-chat'), 'a stale beat is ignored after its process exits')
