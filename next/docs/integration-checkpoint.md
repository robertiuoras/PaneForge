...
## Interrupted PC work remains reviewable

Failed and uncertain requests now retain blocked Review cards with the original request and error, including failures before a remote native identity exists. No local or unrelated native identity is substituted. Stop, shutdown and restart retain uncertainty per request; unsafe automatic retries remain fenced. A later completion for the same request creates its own unread result and resolves the earlier card without deleting its history. Historical GuardDeck notice retraction is not verified.

Verification: 140 unit tests passed, including stop/shutdown late-completion and restart regressions; TypeScript/Vite and minimized parent dev-b build/launch passed. Headless Review checks passed on DESKTOP-CMSUCM1 (12 fixture checks). Logs: `/tmp/next-attention-final-tests.log`, `/tmp/next-attention-build.log`, `/tmp/next-attention-try.log`, `/tmp/next-attention-pc-review.log`. Independent review found two interruption-state defects; both were corrected and covered by the lifecycle regression. A test cleanup race was also corrected by waiting for the provider job to settle before deleting its journal directory.

The approved directory-only Windows `.pytest_cache` ACL repair now passes the normal-user access probe, and fresh sandbox logs no longer report that cache failure. The actual build acceptance still fails at runtime read/execute validation on a separate Codex dependency-cache path. No sandbox setting was weakened and no job was elevated. `/tmp/next-real-build-after-acl.log` and its per-run receipt record this as failure, not implementation completion. Installed replacement acceptance remains outstanding.
