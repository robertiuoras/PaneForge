...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(['POST','PATCH','DELETE'].includes(req.method)){
   ...
   if((match=path.match(/^\/api\/sessions\/([^/]+)\/(turn|steer|stop|resume|link)$/))){const [,id,action]=match;if(action==='stop'){await sessions.stop(id);return send(res,200,{ok:true});}if(action==='resume')return send(res,200,await sessions.resume(id));if(action==='link'){if(!/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(data.taskId||''))throw Error('Taskdriver UUID required');const s=sessions.get(id);s.taskdriverTaskId=data.taskId;sessions.changed();return send(res,200,s);}const session=sessions.get(id);const startingCode=action==='turn'&&explicitBuildIntent(data.text);if(!startingCode&&executionStarting.has(session.id))throw Error('This conversation is starting PC Code work. Wait for its durable result before returning to Chat.');if(startingCode){if(executionStarting.has(session.id)||session.activeTurn||sessions.busy.has(session.id)||hasActiveCode(session.id))throw Error('This conversation is already starting or running work.');executionStarting.add(session.id);}try{if(terminal.state().some(item=>item.sessionId===session.id&&!item.exited&&item.code?.machine==='mac'&&item.code?.kind!=='job'))throw Error('The same conversation is open in a Mac Codex CLI. Exit it before returning to Chat.');if(hasActiveCode(session.id))throw Error('This conversation has active PC Code work. Wait for its durable result before returning to Chat.');if(sessions.providerFor(session)==='codex'){if(typeof data.requestId!=='string'||!data.requestId.trim())throw Error('A request identity is required');if((Object.hasOwn(data,'model')&&data.model!==session.model)||(Object.hasOwn(data,'effort')&&data.effort!==session.effort))throw Error('This session already has a confirmed model and effort. Start a new session to change it.');const live=await ensureCodexReady({refresh:true});const choice=chooseCodexRoute({task:data.text||'',models:live.models,rateLimits:live.rateLimits,requestedModel:session.model,requestedEffort:session.effort});if(!choice.ok)throw Error(choice.reason);session.model=choice.model;session.effort=choice.effort;if(action==='turn'&&explicitBuildIntent(data.text)){if(session.activeTurn||sessions.busy.has(session.id))throw Error('This conversation is running. Wait before starting Code.');const lane=session.laneId?projects.requireLane(session.projectId,session.laneId):projects.laneForCwd(session.projectId,session.cwd);if(!lane||lane.path!==session.cwd)throw Error('This saved lane is no longer available. Reopen the project and choose its current lane.');const run=await terminal.runCodeTurn({sessionId:session.id,projectId:session.projectId,laneId:lane.id,laneName:lane.name,cwd:lane.path,provider:session.provider,model:session.model,effort:session.effort,requestId:data.requestId,text:data.text});return send(res,202,{...run,sessionId:session.id,execution:'pc'});}data={...data,originalText:data.text,text:forgeBuildPrompt(data.text)};}return send(res,200,await sessions.turn(id,data,action==='steer'));}finally{if(startingCode)executionStarting.delete(session.id);}}
   ...
   if(path==='/api/terminal/turn'||path==='/api/terminal/stop'||path==='/api/terminal/launch'){
    ...
    sessions.assertInputAllowed(session.id);if(executionStarting.has(session.id)||session.activeTurn||sessions.busy.has(session.id))throw Error('This conversation is running in Chat. Wait before starting or launching Code.');const nextProject=projects.require('paneforge-next');
    ...
    if(path==='/api/terminal/launch'){if(!['mac','pc'].includes(data.machine))throw Error('Choose Mac or PC');if(data.machine==='mac'){if(session.provider!=='codex'||!session.nativeSessionId||session.activeTurn||sessions.busy.has(session.id))throw Error('Mac CLI requires an idle Codex conversation with its exact native identity');Object.assign(target,{nativeSessionId:session.nativeSessionId,model:session.model,effort:session.effort});}return send(res,200,await terminal.create({...target,machine:data.machine}));}
