 ...
 try{
  ...
  if(['POST','PATCH','DELETE'].includes(req.method)){
   ...
   if((match=path.match(/^\/api\/reviews\/([A-Za-z0-9_-]{1,120})\/(ack|open|reply)$/))&&req.method==='POST'){const [,reviewId,action]=match;
    ...
    if(typeof data.text!=='string'||!data.text.trim()||data.text.length>32000||typeof data.requestId!=='string'||!data.requestId.trim())throw Error('Reply text and request identity are required');const review=reviews.read(reviewId);if(!review){const error=Error('Review not found');error.statusCode=404;throw error;}const session=sessions.get(review.sessionId);if(!session.nativeSessionId||session.nativeSessionId!==review.nativeSessionId){const error=Error('Review is not bound to an active native conversation');error.statusCode=409;throw error;}const dispatched=await dispatchConversationTurn(turnDispatcher,session,data);return send(res,202,{...dispatched.result,sessionId:session.id,...(dispatched.result.execution==='pc'?{}:{nativeSessionId:session.nativeSessionId})});
   }
