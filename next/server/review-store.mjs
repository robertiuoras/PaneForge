...
export class ReviewStore {
  ...
  capturePcTurn(terminal, requestId, session) {
    ...
    if(prior) { if(issue && !issue.resolvedAt) { issue.resolvedAt=new Date().toISOString(); issue.attention=false; issue.resolvedBy=reviewId; this.write(issue); } return prior; }
    const final=request.finalText, blocked=request.outcome==='needs_attention';
    const result=this.record({id:reviewId,sessionId:session.id,nativeSessionId:request.nativeSessionId,execution:'pc',terminalId:terminal.id,kind:blocked?'blocked':'result',proof:final?'claimed':'unverified',report:final?`PC provider execution completed. The retained final outcome follows:\n\n${final}`:'PC provider execution completed, but no final outcome was retained. Completion is unverified.',prompt:request.text,requestId,lane:[code.laneName,code.laneId].filter(Boolean).join(' · '),informational:Boolean(final)&&!blocked,notify:session.kind!=='assistant'&&!/^(automatic-|renew-)/.test(requestId)},{...session,nativeSessionId:request.nativeSessionId,provider:code.provider,cwd:code.checkout});
    if(issue && !issue.resolvedAt) { issue.resolvedAt=new Date().toISOString(); issue.attention=false; issue.resolvedBy=reviewId; this.write(issue); }
    return result;
  }
