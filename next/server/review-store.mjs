...
export class ReviewStore {
  ...
  record(input, session) {
    if (!id(input?.id) || !id(input?.sessionId) || !id(session?.id) || !id(session?.nativeSessionId)) throw Error('Invalid review or native session ID');
  ...
  capturePcTurn(terminal, requestId, session) {
    ...
    if(code?.kind!=='job' || request?.status!=='completed' || !request.nativeSessionId || typeof request.text!=='string') return null;
