...
export function explicitBuildIntent(text) {
  return typeof text === 'string' && /^\s*(?:please\s+)?(?:(?:(?:can|could|would|will)\s+you|(?:i\s+(?:want|need)|i['’]d\s+like)\s+you\s+to)\s+(?:please\s+)?)?(?:build|implement|fix|add|create)\b(?!\s*(?:[?!.]|$))/i.test(text);
}
