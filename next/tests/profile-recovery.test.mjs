...
test('saved profiles reopen without the prototype project and retain terminal project guards',async()=>{
 const root=realpathSync(mkdtempSync(join(tmpdir(),'next-profile-'))),project=join(root,'work'),dataDir=join(root,'state');
 mkdirSync(project);mkdirSync(dataDir);execFileSync('git',['init','--quiet',project]);
 const projectId=createHash('sha256').update(project).digest('hex').slice(0,16);
 writeFileSync(join(dataDir,'projects.json'),JSON.stringify({projects:[{path:project}]}));
 writeFileSync(join(dataDir,'sessions.json'),JSON.stringify([{id:'saved',projectId,cwd:project,nativeSessionId:'native-saved',provider:'codex',items:[],title:'Saved work'},{id:'legacy',cwd:project,nativeSessionId:'native-legacy',provider:'codex',items:[],title:'Unassigned legacy work'}]));
